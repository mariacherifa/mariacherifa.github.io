...
then the new policy assigns exactly the same probability to the token as the old policy.
...
Now suppose the sampled token has positive advantage:

$$
A_t>0.
$$

This means that the token was better than expected, so we want to increase its probability. A standard policy-gradient update would increase

$$
\rho_t(\theta).
$$

But without any constraint, the optimizer could make this ratio very large. PPO prevents this by stopping the objective from improving once the ratio goes above

$$
1+\epsilon.
$$

So for positive advantage, PPO says:

$$
\text{increase the probability of this token, but not by more than about } 1+\epsilon.
$$

Now suppose the sampled token has negative advantage:

$$
A_t<0.
$$

This means that the token was worse than expected, so we want to decrease its probability. A standard policy-gradient update would decrease

$$
\rho_t(\theta).
$$

But again, the optimizer could decrease it too aggressively. PPO prevents this by stopping the objective from improving once the ratio goes below

$$
1-\epsilon.
$$

So for negative advantage, PPO says:

$$
\text{decrease the probability of this token, but not by more than about } 1-\epsilon.
$$

This is implemented through the clipped objective
...
The clipping function restricts the ratio to the interval

$$
[1-\epsilon,1+\epsilon].
$$

The minimum chooses the more conservative value between the unclipped objective and the clipped objective. This is what prevents the update from becoming too large.

So the PPO rule can be summarized as:

$$
\text{make good tokens more likely, but not too much;}
$$

$$
\text{make bad tokens less likely, but not too much.}
$$

This makes policy optimization more stable.

In RLHF, PPO is usually combined with a KL penalty to the reference model. These two mechanisms are related, but they do not play exactly the same role.
